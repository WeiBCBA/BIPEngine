...
def generate_exact_bip_doc(cohort_key, lang_choice):
  ...
  st_zh = (
      ...
      " 模式：讲解、示范、演练与即时反馈)，覆盖全体一线干预人员（RBT/治疗师）。\n•"
      " 督导与忠实度频次：每周使用 10 项干预忠实度核查表进行直接观察评估，确保执行准确率（要求达标率>90%）。\n•"
      " 计划复审周期：每 90 天进行一次正式临床复审，若目标行为发生率出现 20%"
      " 的激增，则立即启动非计划复审。"
  )
  st_es = (
      ...
      " \n• Frecuencia de observación: Observaciones semanales directas usando"
      " lista de verificación de fidelidad (>90%).\n• Revisión del plan:"
      " Revisión clínica cada 90 días o ante aumento del 20% en conductas."
  )

  add_bi_item(
      doc,
      "Staff Training, Implementation Fidelity & Review Schedule",
      st_en,
      (
          "人员培训、干预忠实度与计划复审安排"
          if lang_mode == "zh"
  ...
  add_bi_item(
      ...
      "Tool Positioning & Clinical Responsibility",
      (
          "This plan is generated by an automated tool serving strictly as a"
          " First-Draft Synthesizer. The ultimate clinical judgment, validation"
          " of intervention protocols, and formal signature responsibility"
      ...
      (
          "本计划由自动化工具生成，仅作为初稿合成器 (First-Draft Synthesizer)。"
          "最终的临床判断、干预方案验证以及正式签字确认的临床责任 (Clinical Responsibility)"
          " 完全由审核并定稿该文件的合格行为分析师 (BCBA 或 LBA) 承担。"
          if lang_mode == "zh"
...
# ==========================================
# 9. Main Action Tab Controls (FBA / BIP Generation)
# ==========================================
st.divider()
st.markdown("### 3️⃣ Automated Clinical Synthesis & Report Generation")

tab_fba, tab_bip = st.tabs(
    ["📄 FBA Report Generator (Separated)", "📑 BIP Enriched Draft Generator"]
)

with tab_fba:
  st.markdown("#### Functional Behavioral Assessment (FBA) - Export Suite")
  st.markdown(
      "Select output report language and generate customized behavior-by-behavior"
      " FBA document with dedicated 4-week longitudinal tracking charts."
  )

  fba_lang = st.selectbox(
      "Select FBA Report Output Language:",
      [
          "English (Standard)",
          "Bilingual: English & Chinese (中文双语)",
          "Bilingual: English & Spanish (Español Bilingüe)",
      ],
      key="fba_lang_select",
  )
  ...
  for idx, b in enumerate(active_behaviors):
    b_label = b["name"]
    if fba_lang.startswith("Bilingual: English & Chinese") and "name_zh" in b:
      b_label = f"{b['name']} | {b['name_zh']}"
    elif fba_lang.startswith("Bilingual: English & Spanish") and "name_es" in b:
      b_label = f"{b['name']} | {b['name_es']}"

    if st.checkbox(b_label, value=True, key=f"fba_b_chk_{selected_cohort_key}_{idx}"):
      selected_fba_behaviors.append(b)

  if st.button(
      "🚀 Generate & Download Custom FBA Report (.docx)",
      key=f"btn_gen_fba_{selected_cohort_key}",
  ):
    if not selected_fba_behaviors:
      st.error("Please select at least one target behavior.")
    else:
      with st.spinner(
          "Synthesizing behavior analyses, charts, and triangulation"
          " formulas..."
      ):
        fba_bio = generate_exact_fba_doc(
            selected_cohort_key, fba_lang, selected_fba_behaviors
        )
      st.success(
          "✅ FBA Report Successfully Generated with Dedicated Charts &"
          " Triangulation!"
      )
      st.download_button(
...
with tab_bip:
  st.markdown("#### Behavior Intervention Plan (BIP) - Export Suite")
  st.markdown(
      "Generate comprehensive, enriched BIP draft including proactive"
      " strategies, FCT protocols, DRA reinforcement, extinction, and staff"
      " training."
  )

  bip_lang = st.selectbox(
      "Select BIP Report Output Language:",
      [
          "English (Standard)",
          "Bilingual: English & Chinese (中文双语)",
          "Bilingual: English & Spanish (Español Bilingüe)",
  ...
  if st.button(
      "🚀 Generate & Download Comprehensive BIP Draft (.docx)",
      key=f"btn_gen_bip_{selected_cohort_key}",
  ):
    with st.spinner(
        "Synthesizing proactive plans, FCT protocols, and crisis safety"
        " procedures..."
    ):
      bip_bio = generate_exact_bip_doc(selected_cohort_key, bip_lang)
    st.success("✅ Comprehensive BIP Draft Successfully Generated!")
    st.download_button(
        label="📥 Click Here to Download BIP Draft (.docx)",
        data=bip_bio,
        file_name=f"BIP_Draft_{current_meta['file_tag']}.docx",
        mime=(
            "application/vnd.openxmlformats-officedocument.wordprocessingml.document"
        ),
    )
