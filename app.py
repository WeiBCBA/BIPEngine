...
def generate_exact_bip_doc(cohort_key, lang_choice):
  ...
  st_en = (
      ...
      " weekly observations utilizing..."
  )
  st_zh = (
      "• 责任人：认证行为分析师 (BCBA) / 持牌行为分析师 (LBA)。\n•"
      " 培训流程：所有直接干预人员（RBT/治疗师）需完成最初 2 小时的理论模块，并接受 1:1"
      " 行为技能训练 (BST: 指导、建模、演练与反馈)。\n• 观察与忠实度频次：每周使用..."
  )
  st_es = (
      "• Responsable: BCBA / LBA.\n• Proceso de capacitación: Módulo didáctico"
      " inicial de 2 horas seguido de entrenamiento en habilidades"
      " conductuales (BST).\n• Frecuencia de observación: Semanal..."
  )

  add_bi_item(
      doc,
      "Staff Training & Monitoring Protocols",
      st_en,
      (
          "人员培训与监督协议"
          if lang_mode == "zh"
          else "Protocolos de capacitación"
          if lang_mode == "es"
          else None
      ),
...
# ==========================================
# 9. Main Action Layout (Tabs & Generators)
# ==========================================
st.divider()
st.markdown("### 3️⃣ Automated Clinical Document Generation & Export")

tab_fba, tab_bip = st.tabs(
    ["📄 FBA Report Generator", "📋 BIP Comprehensive Generator"]
)

with tab_fba:
  st.markdown("#### Functional Behavioral Assessment (FBA) Report Builder")
  st.markdown(
      "Select your preferred document export language and target behaviors to"
      " include in the custom FBA report:"
  )

  fba_lang_choice = st.selectbox(
      "Select FBA Report Language Format:",
      options=[
          "English (Standard)",
  ...
  st.markdown(
      "**Select Target Behaviors to Include in FBA Report:**"
  )
  fba_selected_behaviors = []
  for idx, b in enumerate(active_behaviors):
    chk = st.checkbox(
        f"Include Behavior #{idx+1}: {b['name']}",
        value=True,
        key=f"fba_b_chk_{selected_cohort_key}_{idx}",
    )
    if chk:
      fba_selected_behaviors.append(b)
  ...
  if st.button(
    ...
    if not fba_selected_behaviors:
      st.error(
          "Please select at least one target behavior to generate the FBA"
          " report."
      )
    else:
      with st.spinner(
          "Synthesizing triangulated FBA data and formatting Word document..."
      ):
        fba_bio = generate_exact_fba_doc(
            selected_cohort_key, fba_lang_choice, fba_selected_behaviors
        )
      st.success(
          "✅ FBA Report Draft successfully formulated and formatted!"
      )
      st.download_button(
          label="📥 Download Final FBA Report (.docx)",
          data=fba_bio,
...
with tab_bip:
  st.markdown("#### Behavior Intervention Plan (BIP) Comprehensive Builder")
  st.markdown(
      "Generate the complete Behavior Intervention Plan (BIP) incorporating"
      " FCT, DRA, Extinction, Crisis Protocols, and Staff Training:"
  )

  bip_lang_choice = st.selectbox(
      "Select BIP Document Language Format:",
      options=[
          "English (Standard)",
          "Bilingual (English + Chinese 简体中文)",
          "Bilingual (English + Spanish Español)",
  ...
  if st.button(
      "🚀 Generate & Download Comprehensive BIP (.docx)",
      key="btn_gen_bip",
      use_container_width=True,
  ):
    with st.spinner(
        "Formulating comprehensive Behavior Intervention Plan (BIP)..."
    ):
      bip_bio = generate_exact_bip_doc(selected_cohort_key, bip_lang_choice)
    st.success(
        "✅ Behavior Intervention Plan (BIP) successfully formulated!"
    )
    st.download_button(
        label="📥 Download Final BIP Document (.docx)",
        data=bip_bio,
