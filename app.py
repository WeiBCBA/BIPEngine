...
def generate_behavior_tracking_chart(cohort_key, behavior_index):
  ...
  ax.set_xlabel("Observation Period (4 Weeks)", fontsize=8)
  ax.set_ylabel("Episodes / Total", fontsize=8)
...
st.markdown(
    ...
    <div class="hipaa-banner">
        ...
        <div class="hipaa-body">
            This tool strictly complies with HIPAA privacy regulations and BACB Ethics standards. All data parsing, analysis, and document formulation occur <b>100% locally within your active browser session memory</b>.
        </div>
    </div>
""",
    unsafe_allow_html=True,
...
def generate_exact_fba_doc(cohort_key, lang_choice, behavior_list):
  ...
  add_bi_item(
      ...
      (
          ...
          " Behavior Tracking Document across 4 Weeks).\n\n1. Direct ABC Data: Continuous"
  ...
  add_bi_heading(
      doc,
      1,
      "4. Individual Target Behavior Functional Analyses (4-Week Trends)",
      (
          "4. 目标行为独立功能分析 (按行为逐项拆解与 4 周专属趋势图)"
          if lang_mode == "zh"
          else "4. Análisis funcional individual por conducta (tendencias de 4 semanas)"
          if lang_mode == "es"
          else None
      ),
  )
  ...
  for idx, b in enumerate(behavior_list, 1):
    ...
    doc.add_paragraph(
        f"Note: The chart above illustrates the independent 4-week tracking frequency trend specifically recorded for Target Behavior #{idx}."
    )
  ...
  add_bi_item(
      ...
      (
          "This report is generated by an automated tool serving strictly as a"
          " First-Draft Synthesizer. The ultimate clinical judgment, validation"
          " of data, and formal signature responsibility (Clinical"
          " Responsibility) rest entirely with the qualified Board Certified"
          " Behavior Analyst (BCBA) or Licensed Behavior Analyst (LBA)"
          " reviewing and finalizing this document."
      ),
      (
          "工具定位与临床责任"
          if lang_mode == "zh"
          else "Posicionamiento y responsabilidad"
          if lang_mode == "es"
          else None
      ),
      (
          "本报告由自动化工具生成，仅作为初稿合成器 (First-Draft Synthesizer)。"
          "最终的临床判断、数据验证以及正式签字确认的临床责任 (Clinical Responsibility)"
          " 完全由审核并定稿该文件的合格行为分析师 (BCBA 或 LBA) 承担。"
          if lang_mode == "zh"
          else "Este informe es un sintetizador de primer borrador. La responsabilidad clínica final recae en el BCBA o LBA."
          if lang_mode == "es"
          else None
      ),
      lang_mode,
...
def generate_exact_bip_doc(cohort_key, lang_choice):
  ...
  st_zh = (
      "• 责任人：注册行为分析师 (BCBA) / 执照行为分析师 (LBA)。\n• 培训流程：进行为期"
      " 2 小时的理论培训模块，随后与所有一线干预人员（RBT/治疗师）开展 1:1 行为技能培训（BST：指导、建模、排练与反馈）。\n•"
      " 观察与忠实度频次：每周使用 10 项‘干预执行忠实度核查表’进行直接观察，确保实施准确率（要求达成率 >90%）。\n•"
      " 计划复审周期：每 90 天进行一次正式的临床复审；若目标行为发生频率出现 20%"
      " 的激增，则立即进行复审。"
  )
  st_es = (
      "• Responsable: BCBA / LBA.\n• Proceso de capacitación: Módulo teórico"
      " de 2 horas seguido de entrenamiento en habilidades conductuales (BST)."
      " \n• Frecuencia de supervisión: Observaciones semanales con lista de"
      " verificación de fidelidad (>90%).\n• Revisión del plan: Cada 90 días o"
      " ante un aumento del 20% en la conducta."
  )
  ...
  add_bi_item(
      ...
      (
          "人员培训、忠实度与计划复审"
          if lang_mode == "zh"
          else "Capacitación, fidelidad y revisión"
          if lang_mode == "es"
          else None
      ),
      (
  ...
  # Section 9: Clinical Responsibility Notice
  add_bi_heading(
      doc,
      1,
  ...
  add_bi_item(
      ...
      "Tool Positioning & Clinical Responsibility",
      (
          "This Behavior Intervention Plan (BIP) draft is generated by an"
          " automated tool serving strictly as a First-Draft Synthesizer. The"
          " ultimate clinical judgment, validation of data, and formal"
          " signature responsibility (Clinical Responsibility) rest entirely"
          " with the qualified Board Certified Behavior Analyst (BCBA) or"
          " Licensed Behavior Analyst (LBA) reviewing and finalizing this"
          " document."
      ),
      (
          "工具定位与临床责任"
      ...
      (
          "本行为干预计划 (BIP) 初稿由自动化工具生成，仅作为初稿合成器 (First-Draft Synthesizer)。"
          "最终的临床判断、数据验证以及正式签字确认的临床责任 (Clinical Responsibility)"
          " 完全由审核并定稿该文件的合格行为分析师 (BCBA 或 LBA) 承担。"
          if lang_mode == "zh"
          else "Este borrador de BIP es un sintetizador de primer borrador. La responsabilidad clínica final recae en el BCBA o LBA."
          if lang_mode == "es"
          else None
      ),
      lang_mode,
...
# ==========================================
# 9. Main Workflow Tabs & Interactive Output
# ==========================================
st.divider()

tab_fba, tab_bip = st.tabs([
    "📑 FBA Draft Synthesizer & Export",
    "📋 BIP Draft Synthesizer & Export",
)

with tab_fba:
  st.markdown("### 📄 Functional Behavioral Assessment (FBA) Report Generator")
  st.markdown(
      "Select your preferred report export language and target behaviors to"
      " generate a customized, de-identified FBA report draft (.docx) with"
      " embedded 4-week tracking charts."
  )

  col_fba1, col_fba2 = st.columns([1, 1])

  with col_fba1:
    fba_lang = st.selectbox(
        "Select Export Language / 导出报告语言:",
        options=[
            "English (Standard Professional)",
            "Chinese / 中英双语对照 (Bilingual: English + 中文)",
            "Spanish / Español Bilingual (English + Español)",
        ],
        index=1,
        key="fba_lang_select",
    )
  ...
  with col_fba2:
    st.markdown("#### Select Target Behaviors to Include in FBA:")
    include_b1 = st.checkbox(
        f"Include Behavior #1: {active_behaviors[0]['name']}",
        value=True,
        key="fba_b1",
    )
    include_b2 = st.checkbox(
        f"Include Behavior #2: {active_behaviors[1]['name']}",
        value=True,
        key="fba_b2",
    )
    include_b3 = (
        st.checkbox(
            f"Include Behavior #3: {active_behaviors[2]['name']}",
            value=True,
            key="fba_b3",
        )
  ...
  if include_b1:
    selected_behaviors_for_fba.append(active_behaviors[0])
  if include_b2:
    selected_behaviors_for_fba.append(active_behaviors[1])
  if len(active_behaviors) > 2 and include_b3:
    selected_behaviors_for_fba.append(active_behaviors[2])

  if not selected_behaviors_for_fba:
    st.warning("⚠️ Please select at least one target behavior to generate FBA.")
  else:
    fba_docx_io = generate_exact_fba_doc(
        selected_cohort_key, fba_lang, selected_behaviors_for_fba
    )
    st.download_button(
        label=(
            "📥 Download Completed FBA Report Draft (.docx) [Triangulated &"
            " Charted]"
        ),
        data=fba_docx_io,
        file_name=(
            f"BCBA_FBA_Report_{current_meta['file_tag']}_{selected_cohort_key}.docx"
        ),
        mime=(
            "application/vnd.openxmlformats-officedocument.wordprocessingml.document"
        ),
...
with tab_bip:
  st.markdown("### 📋 Behavior Intervention Plan (BIP) Generator")
  st.markdown(
      "Generate a comprehensive, evidence-based Behavior Intervention Plan"
      " (.docx) incorporating proactive antecedent modifications, FCT"
      " replacement strategies, DRA reinforcement, crisis safety protocols, and"
      " staff training guidelines."
  )

  bip_lang = st.selectbox(
      "Select BIP Export Language / BIP 导出报告语言:",
      options=[
          "English (Standard Professional)",
          "Chinese / 中英双语对照 (Bilingual: English + 中文)",
          "Spanish / Español Bilingual (English + Español)",
  ...
  bip_docx_io = generate_exact_bip_doc(selected_cohort_key, bip_lang)
  st.download_button(
      label=(
          "📥 Download Comprehensive BIP Draft (.docx) [Enriched with Crisis &"
          " Training Protocols]"
      ),
      data=bip_docx_io,
      file_name=(
          f"BCBA_BIP_Plan_{current_meta['file_tag']}_{selected_cohort_key}.docx"
      ),
      mime=(
          "application/vnd.openxmlformats-officedocument.wordprocessingml.document"
      ),
...
st.markdown(
    ...
    "BCBA Clinical FBA & BIP Draft Formulation Tool | Designed strictly for"
    " Professional Behavior Analysts under HIPAA & BACB Ethics Standards."
    "</div>",
    unsafe_allow_html=True,
)
