...
def generate_exact_fba_doc(cohort_key, lang_choice, behavior_list):
  ...
  add_bi_item(
      doc,
      "Data Sources & Weighting Weight",
      (
          "• Direct ABC Observations (65% Weighting)\n• Indirect Stakeholder"
          " Interviews (25% Weighting)\n• Psychometric QABF Rating Scales"
          " (10% Weighting)\n\nTriangulation Algorithm: Data synthesis is"
          " calculated using 65% Direct ABC + 25% Indirect Interview + 10%"
          " QABF Assessment."
      ),
      (
          "数据来源与权重算法"
          if lang_mode == "zh"
          else "Fuentes de datos y algoritmo de triangulación"
          if lang_mode == "es"
          else None
      ),
      (
          "• 直接 ABC 观察记录 (权重 65%)\n• 利益相关者访谈 (权重 25%)\n• 心理测量"
          " QABF 量表评估 (权重 10%)\n\n交叉验证算法标准公式：数据综合结论基于 65%"
          " 直接 ABC 数据 + 25% 间接访谈 + 10% QABF 量表进行精确比对验证。"
          if lang_mode == "zh"
          else "• Observaciones directas ABC (Peso 65%)\n• Entrevistas a"
          " partes interesadas (Peso 25%)\n• Escalas psicométricas QABF (Peso"
          " 10%)\n\nAlgoritmo de triangulación: Síntesis de datos calculada"
          " mediante 65% ABC directo + 25% Entrevista + 10% QABF."
      ),
      lang_mode,
  )
