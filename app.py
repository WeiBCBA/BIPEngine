        ...
        "behaviors": [
            {
                ...
                "def_zh": "拒绝组装或分类任务，并伴随大声言语威胁（>75 dB）或推开工作材料的行为。以言语发作或推开材料为行为开始，以持续 3 分钟安静参与任务为行为结束。",
...
def generate_exact_bip_doc(cohort_key, lang_choice):
  ...
  st_zh = (
      "• 责任人：认证行为分析师 (BCBA) / 执照行为分析师 (LBA)。\n• 培训流程：进行为期 2"
      " 小时的专题理论培训，随后对所有直接干预人员（RBT/治疗师）开展 1:1"
      " 行为技能训练（BST：指令、建模、演练与反馈）。\n• 观察与忠实度频次：使用 10"
      " 项干预忠实度核查表每周进行直接观察，确保执行准确率（要求达标率>90%）。\n•"
      " 计划复审周期：每 90"
      " 天进行一次正式临床复审，若目标行为发生频率激增 20% 则立即启动复审。"
  )
  st_es = (
      "• Responsable: BCBA / LBA.\n• Proceso de capacitación: Módulo didáctico"
      " de 2 horas seguido de entrenamiento en habilidades conductuales (BST).\n•"
      " Frecuencia de observación: Observaciones semanales con lista de"
      " verificación (>90% de cumplimiento).\n• Revisión del plan: Formal cada"
      " 90 días o inmediatamente si hay un aumento del 20%."
  )

  add_bi_item(
      doc,
      "Staff Training, Fidelity & Review Schedule",
      st_en,
      (
          "人员培训、忠实度与复审周期"
          if lang_mode == "zh"
  ...
  add_bi_item(
      ...
      "Tool Positioning & Clinical Responsibility",
      (
          "This Behavior Intervention Plan (BIP) is generated by an automated"
          " tool serving strictly as a First-Draft Synthesizer. The ultimate"
          " clinical judgment, validation of data, and formal signature"
          " responsibility (Clinical Responsibility) rest entirely with the"
          " qualified Board Certified Behavior Analyst (BCBA) or Licensed"
          " Behavior Analyst (LBA) reviewing and finalizing this document."
      ),
      (
          "工具定位与临床责任"
          if lang_mode == "zh"
      ...
      (
          "本行为干预计划 (BIP) 由自动化工具生成，仅作为初稿合成器 (First-Draft Synthesizer)。"
          "最终的临床判断、数据验证以及正式签字确认的临床责任 (Clinical Responsibility)"
          " 完全由审核并定稿该文件的合格行为分析师 (BCBA 或 LBA) 承担。"
          if lang_mode == "zh"
          else "Este Plan de Intervención Conductual es un sintetizador de primer borrador. La responsabilidad clínica final recae en el BCBA o LBA."
          if lang_mode == "es"
          else None
      ),
      lang_mode,
...
# ==========================================
# 9. Main Action Tabs & Document Generators
# ==========================================
st.divider()
st.markdown("### 3️⃣ Automated Clinical Document Synthesis & Export")

lang_choice = st.selectbox(
    "Select Document Output Language / 双语报告导出语言:",
    [
        "English (EN Only)",
        "Bilingual: English + Chinese (中英双语报告)",
        "Bilingual: English + Spanish (Español / Inglés)",
    ],
    index=1,
)

doc_type_choice = st.radio(
    "Select Document Type to Generate & Preview:",
    [
        "📄 Functional Behavioral Assessment (FBA) Report",
...
with col_gen1:
  if "FBA" in doc_type_choice:
    st.info(
        "📄 **FBA Report Generator Active:** Will synthesize data for all"
        f" **{len(active_behaviors)} target behaviors** associated with"
        f" {current_meta['title']} into a structured, publication-ready Word"
        " document."
    )
    if st.button(
      ...
      with st.spinner("Synthesizing FBA report and generating charts..."):
        fba_bio = generate_exact_fba_doc(
            selected_cohort_key, lang_choice, active_behaviors
        )
      st.success("✅ FBA Report successfully generated!")
      st.download_button(
          label="📥 Click Here to Download FBA Report (.docx)",
          data=fba_bio,
          file_name=f"BCBA_FBA_Report_{current_meta['file_tag']}.docx",
          mime=(
              "application/vnd.openxmlformats-officedocument.wordprocessingml.document"
          ),
          use_container_width=True,
    ...
    st.info(
        "📋 **BIP Generator Active:** Will synthesize a comprehensive Behavior"
        f" Intervention Plan for {current_meta['title']} with proactive"
        " protocols, FCT, DRA, extinction, and crisis safety guidelines."
    )
    if st.button(
      ...
      with st.spinner("Synthesizing BIP report and safety protocols..."):
        bip_bio = generate_exact_bip_doc(selected_cohort_key, lang_choice)
      st.success("✅ BIP Report successfully generated!")
      st.download_button(
          label="📥 Click Here to Download BIP Report (.docx)",
          data=bip_bio,
          file_name=f"BCBA_BIP_Plan_{current_meta['file_tag']}.docx",
          mime=(
              "application/vnd.openxmlformats-officedocument.wordprocessingml.document"
          ),
          use_container_width=True,
...
with col_gen2:
  ...
  st.markdown(
      "<small>After downloading your .docx draft, use <b>Ctrl+H</b> in Microsoft"
      " Word to instantly replace bracketed placeholders like"
      " <code>[CLIENT_NAME]</code> and <code>[CLIENT_ID]</code> with your actual"
